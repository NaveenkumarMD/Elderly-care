...
class Findhelpertravel extends Component {
    constructor(props) {
        super(props)
        this.state = {
            modalVisible:false,
    ...
    notify = async () => {
        ...
        this.state.token.forEach(async data => {
            await sendPushNotification(data, { title: 'Travel', body: `${this.state.userdata.name} is asking a help to take him to ${this.state.routedata.location} on${this.state.routedata.traveldate}` })
        })
        ...
        firebase.firestore().collection('Travel').add({
    ...
    async componentDidMount() {
        ...
        await firebase.firestore().collection('Helpers').where('interested', 'array-contains', 'Travel').get().then(query => {
            var arr = []
            var tokenarray = []
            var helpersid = []
        ...
        const Item = ({ title, distance, mobile, rating }) => (
            <TouchableOpacity style={styles.item} onPress={() => this.handleviewhelper(mobile)}>
                <View >
                    ...
                    <Text style={{ color: 'dodgerblue' }}>{distance} km</Text>
                </View>
...
export default Findhelpertravel
