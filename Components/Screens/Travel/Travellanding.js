...
class Travellanding extends Component {
    ...
    componentDidMount=async()=>{
        ...
        await firebase.firestore().collection('Travel').where('Elderid','==',this.state.userdata.mobile).onSnapshot(querysnapshot=>{
            var array=[]
    ...
    render() {
        ...
        const requests=this.state.requests.map(data=>{
            if(!data.status.Completed){
                a+=1
                return(
                    <TouchableOpacity activeOpacity={0.5} style={{padding:15,flexDirection:'row',justifyContent:'space-between',borderBottomColor:'dodgerblue',borderBottomWidth:1}}
                    onPress={()=>this.props.navigation.navigate('Viewrequesttravel',{data:data.id})}
                    >
                        <View>
